        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden mb-8">
            ...
            <div class="bg-becs-navy text-white px-6 py-4 flex items-center gap-3">
                ...
                <h2 class="text-xl font-semibold">
                    {{ isset($permissionToEdit) ? 'Edit Permission' : 'Create Permission' }}
                </h2>
            </div>
            ...
            <!-- Form -->
            <form action="{{ isset($permissionToEdit) ? route('admin.permissions.update', $permissionToEdit->id) : route('admin.permissions.store') }}"
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" placeholder="Enter permission name (e.g., approve petty cash)"
                           class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-becs-navy"
                           value="{{ $permissionToEdit->name ?? old('name') }}" required>
                    @error('name')
                ...
                <button type="submit"
                    ...
                    <i class="fa-solid {{ isset($permissionToEdit) ? 'fa-pen' : 'fa-plus' }}"></i>
                    {{ isset($permissionToEdit) ? 'Update Permission' : 'Create Permission' }}
                </button>
            </form>
        </div>
            ...
            <table class="min-w-full table-auto border-collapse">
                <thead class="bg-becs-navy text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium uppercase tracking-wider">Permission Name</th>
                ...
                <tbody>
                    @foreach($permissions as $permission)
                        <tr class="hover:bg-gray-100 even:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $permission->name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $permission->created_at->format('Y-m-d H:i') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $permission->updated_at->format('Y-m-d H:i') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700 flex gap-2">
                                <a href="{{ route('admin.permissions.edit', $permission->id) }}" class="bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600">
